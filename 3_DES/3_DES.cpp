...
public:
	...
	void keyGenerate(byte * keyBytes)
	{
		...
		rnd.GenerateBlock(keyBytes, CryptoPP::DES::DEFAULT_KEYLENGTH);
	...
	{
		CryptoPP::AutoSeededRandomPool rnd;
		rnd.GenerateBlock(ivBytes, CryptoPP::DES::BLOCKSIZE);
	...
	int Encryption()
	{
		...
		byte ivBytes[CryptoPP::DES::BLOCKSIZE];
		...
		else if (mode == 2)
		{
			...
			CryptoPP::CBC_Mode< CryptoPP::DES>::Encryption Enc;
		...
		else if (mode == 3)
		{
			...
			keyGenerate(keyBytes); ivGenerate(ivBytes);
			CryptoPP::CFB_Mode< CryptoPP::DES>::Encryption Enc;
			Enc.SetKeyWithIV(keyBytes, sizeof(keyBytes), ivBytes);
			CryptoPP::ArraySource(plaintext.GetData().data(), plaintext.GetData().size(), true,
				new CryptoPP::StreamTransformationFilter(Enc, new CryptoPP::Redirector(cs)));
		}
		...
		else if (mode == 4)
		{
			...
			keyGenerate(keyBytes); ivGenerate(ivBytes);
			CryptoPP::OFB_Mode< CryptoPP::DES>::Encryption Enc;
			Enc.SetKeyWithIV(keyBytes, sizeof(keyBytes), ivBytes);
			CryptoPP::ArraySource(plaintext.GetData().data(), plaintext.GetData().size(), true,
				new CryptoPP::StreamTransformationFilter(Enc, new CryptoPP::Redirector(cs)));
		}
		...
		else if (mode == 5)
		{
			...
			keyGenerate(keyBytes); ivGenerate(ivBytes);
			CryptoPP::CTR_Mode< CryptoPP::DES>::Encryption Enc;
			Enc.SetKeyWithIV(keyBytes, sizeof(keyBytes), ivBytes);
			CryptoPP::ArraySource(plaintext.GetData().data(), plaintext.GetData().size(), true,
				new CryptoPP::StreamTransformationFilter(Enc, new CryptoPP::Redirector(cs)));
		}
	...
	{
		byte randomKey[CryptoPP::DES::DEFAULT_KEYLENGTH];
		byte randomIv[CryptoPP::DES::BLOCKSIZE];
		...
		dec_plaintext.GetData().resize(ciphertext.GetData().size() + CryptoPP::DES::BLOCKSIZE);
		...
		{
			keyGet(randomKey); ivGet(randomIv);
			CryptoPP::CFB_Mode< CryptoPP::DES>::Decryption Dec;
			Dec.SetKeyWithIV(randomKey, sizeof(randomKey), randomIv);
			CryptoPP::ArraySource(ciphertext.GetData().data(), ciphertext.GetData().size(), true,
				new CryptoPP::StreamTransformationFilter(Dec, new CryptoPP::Redirector(ds)));
		}
		...
		{
			keyGet(randomKey); ivGet(randomIv);
			CryptoPP::OFB_Mode< CryptoPP::DES>::Decryption Dec;
			Dec.SetKeyWithIV(randomKey, sizeof(randomKey), randomIv);
			CryptoPP::ArraySource(ciphertext.GetData().data(), ciphertext.GetData().size(), true,
				new CryptoPP::StreamTransformationFilter(Dec, new CryptoPP::Redirector(ds)));
		}
		...
		{
			keyGet(randomKey); ivGet(randomIv);
			CryptoPP::CTR_Mode< CryptoPP::DES>::Decryption Dec;
			Dec.SetKeyWithIV(randomKey, sizeof(randomKey), randomIv);
			CryptoPP::ArraySource(ciphertext.GetData().data(), ciphertext.GetData().size(), true,
				new CryptoPP::StreamTransformationFilter(Dec, new CryptoPP::Redirector(ds)));
		}
...
public:
	...
	{
		CryptoPP::AutoSeededRandomPool rnd;
		rnd.GenerateBlock(keyBytes, CryptoPP::DES_EDE3::DEFAULT_KEYLENGTH);
	...
	{
		CryptoPP::AutoSeededRandomPool rnd;
		rnd.GenerateBlock(ivBytes, CryptoPP::DES_EDE3::BLOCKSIZE);
	...
	int Encryption()
	{
		...
		byte ivBytes[CryptoPP::DES_EDE3::BLOCKSIZE];
		...
		else if (mode == 4)
		{
			std::cout << "Encryption in OFB mode in process" << std::endl;
			keyGenerate(keyBytes); ivGenerate(ivBytes);
			CryptoPP::OFB_Mode< CryptoPP::DES_EDE3 >::Encryption Enc;
			Enc.SetKeyWithIV(keyBytes, sizeof(keyBytes), ivBytes);
			CryptoPP::ArraySource(plaintext.GetData().data(), plaintext.GetData().size(), true,
				new CryptoPP::StreamTransformationFilter(Enc, new CryptoPP::Redirector(cs)));
		}
		...
		else if (mode == 5)
		{
			std::cout << "Encryption in CTR mode in process" << std::endl;
			keyGenerate(keyBytes); ivGenerate(ivBytes);
			CryptoPP::CTR_Mode< CryptoPP::DES_EDE3 >::Encryption Enc;
			Enc.SetKeyWithIV(keyBytes, sizeof(keyBytes), ivBytes);
			CryptoPP::ArraySource(plaintext.GetData().data(), plaintext.GetData().size(), true,
				new CryptoPP::StreamTransformationFilter(Enc, new CryptoPP::Redirector(cs)));
		}
	...
	void Decryption(int mode)
	{
		byte randomKey[CryptoPP::DES_EDE3::DEFAULT_KEYLENGTH];
		byte randomIv[CryptoPP::DES_EDE3::BLOCKSIZE];
		...
		dec_plaintext.GetData().resize(ciphertext.GetData().size() + CryptoPP::DES_EDE3::BLOCKSIZE);
		CryptoPP::ArraySink ds(&dec_plaintext.GetData()[0], dec_plaintext.GetData().size());
		...
		{
			keyGet(randomKey); ivGet(randomIv);
			CryptoPP::OFB_Mode< CryptoPP::DES_EDE3 >::Decryption Dec;
			Dec.SetKeyWithIV(randomKey, sizeof(randomKey), randomIv);
			CryptoPP::ArraySource(ciphertext.GetData().data(), ciphertext.GetData().size(), true,
				new CryptoPP::StreamTransformationFilter(Dec, new CryptoPP::Redirector(ds)));
		}
		...
		{
			keyGet(randomKey); ivGet(randomIv);
			CryptoPP::CTR_Mode< CryptoPP::DES_EDE3 >::Decryption Dec;
			Dec.SetKeyWithIV(randomKey, sizeof(randomKey), randomIv);
			CryptoPP::ArraySource(ciphertext.GetData().data(), ciphertext.GetData().size(), true,
				new CryptoPP::StreamTransformationFilter(Dec, new CryptoPP::Redirector(ds)));
		}
...
int main()
{
	DES object;	TripleDES object3;
	std::cout << "Select mode: 1)DES, 2)Triple DES" << std::endl;
	int des_mode;
	int mode;
	std::cin >> des_mode;
	if (des_mode == 1)
	{
		mode = object.Encryption();
		object.Decryption(mode);
	}
	else
		if (des_mode == 2)
		{
			mode = object3.Encryption();
			object3.Decryption(mode);
		}
	system("pause");
}